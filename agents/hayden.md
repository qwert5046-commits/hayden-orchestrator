...
### Phase 1 종료 시점 한정: 의도 정합성 자가 점검 (1 회만 발동)

Phase 1 이 끝나면 (Step 6 직후) 다음 자가 점검을 한 번만 수행한다. **목적**: 야간 자율 진행 중 아키텍처 근본 결정이 PRD 의도와 어긋난 채로 phase 가 누적되는 걸 일찍 잡는 안전망.

**판단 절차**:

1. `docs/PRD.md` (또는 자동 탐지된 PRD 파일) 의 핵심 요건 3~5 줄 추출
2. `docs/plans/phase-1/` 의 spec 과 실제 commit 된 변경 파일 목록 대조
3. 다음 질문에 너 스스로 답:
   - "Phase 1 결과물이 PRD 의 핵심 의도를 향해 가고 있나?" (방향)
   - "Phase 1 의 아키텍처 선택이 Phase 2+ 에서 되돌리기 어려운가?" (되돌림 비용)
   - "PRD 가 명시한 사용자 페르소나 / 성공 지표와 spec 이 합치하나?" (정합성)

**결과 처리**:

- ✅ **의도 일치** → `docs/WORK_LOG.md` 에 `Phase 1 의도 점검 ✓` 한 줄. 멈추지 않고 Phase 2 진입.
- 🚨 **의도 차이 큼** (3 질문 중 1 개 이상 "No" 또는 "확신 없음"):
  - `docs/DECISIONS.md` 최상단에 🚨 박스로 "Phase 1 의도 정합성 우려" 항목 신설 (어긋난 부분 + 추측되는 영향 + 권장 사용자 확인 사항)
  - `MORNING_REPORT.md` 의 TL;DR 1 번 자리에 자동 노출 ("오늘 안 보면 큰일")
  - **그래도 멈추지 않고 Phase 2 진입** ([L-010](../lessons/L-010-no-asking-policy.md) — 사용자 깨우지 않음 정책 유지). 사용자가 아침에 결정.

**왜 Phase 1 만**:

- 매 phase 발동 = 컨텍스트·시간 낭비 (이후 phase 는 [reviewer](reviewer.md) + 막힘 사다리가 커버)
- Phase 1 = "기본 골격이 잡히는 시점" → 의도 어긋남 감지 효과가 가장 큼
- Phase 2+ 에서 새 사용자 결정 항목이 발생하면 별도로 `DECISIONS.md` 에 큐잉됨
