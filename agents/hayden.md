...
- 예외 3: 사용자가 명시적으로 "Phase X 끝나면 멈춰" 등 일시 정지 신호를 줬을 때만 멈춤
- 통합 테스트(manual_test 등)가 사용자 작업이라 미완료여도 다음 phase의 **코드 작성은 진행**한다 (인터페이스 의존만 있으므로). 통합 검증은 Phase 3 dry_run 또는 사용자가 깨어났을 때 진행.
